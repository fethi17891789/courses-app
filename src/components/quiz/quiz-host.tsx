...
export function QuizHost({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => {
    if (!data || data.session?.status !== "leaderboard") return;
    const qIdx = data.session.current_question_index;
  ...
  if (session.status === "leaderboard") {
    ...
    // "old" phase → display in previous order so cards visibly travel to new positions
